...
@router.get("")
async def list_campaigns_disabled(current_user: dict = require_roles("enterprise_staff")):
...
@router.post("")
async def create_campaign_disabled(current_user: dict = require_roles("enterprise_staff")):
    """Campaigns are disabled - use VulnForge Lab Chat instead"""
    raise HTTPException(
        status_code=501,
        detail="Campaign system disabled. Use /api/lab/chat for individual lab generation."
...
async def get_campaign_disabled(campaign_id: str, current_user: dict = require_roles("enterprise_staff")):
    """Campaigns are disabled - use VulnForge Lab Chat instead"""
    raise HTTPException(
        status_code=501,
        detail="Campaign system disabled. Use /api/lab/chat for individual lab generation."
